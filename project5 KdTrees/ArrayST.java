...
public class ArrayST<Key, Value> implements BasicST<Key, Value> {
    ...
    public void delete(Key key) {
        ...
        // look for the key ,save the index
        // if key found, shift all entires to left
        // decrement n and resize if nessery
        if (contains(key)) {
            int i = 0;
            for (i = 0; i < n; i++) { // looking for the index of key
                if (keys[i].equals(key)) {
            ...
            for (int j = i; j < n -1; j++) {  // shift all entires to left
                keys[j] = keys[j+1];
                values[j] = values[j+1];
            }
